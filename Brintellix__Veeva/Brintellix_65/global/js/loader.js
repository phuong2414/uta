...
require(['breadcrumb', 'toolbar', 'marker', 'sitemap', 'circle', 'button', 'drawing', 'increasenumber', 'popup', 'srt', 'video', 'slideshow', 'coverflow', 'callsummary', 'survey', 'menu', 'stylemenu', 'ref', 'custom', 'jquery.ui.core', 'jquery.ui.widget', 'jquery.ui.mouse', 'jquery.ui.slider', 'jquery.ui.draggable', 'jquery.ui.droppable', 'jquery.ui.effect', 'jquery.ui.effect-slide', 'jquery.ui.effect-blind', 'jquery.ui.effect-fade'], function() {
    var conf = drcom.config,
        externalScripts = [],
        fonts = conf.preloadFonts;

    function removeLoadingPage() {
        if (drcom.isPhantom) return;
        setTimeout(function() {$('#loadingPage').remove();}, 100);
    }
    ...
    } else removeLoadingPage();
    
    // load external scripts
    if (conf.externalScripts && conf.externalScripts.length) {
        externalScripts = conf.externalScripts;
    }
    externalScripts.push('global');
    if (drcom.isPhantom) externalScripts = [];
    
    require(externalScripts, function() {
