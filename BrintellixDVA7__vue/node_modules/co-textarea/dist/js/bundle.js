...
var Textarea = require('./src/js/textarea').Textarea,
...
module.exports = createComponent({
	...
	localizationTemplate: resolve('i18n/{lang}.json')
			...
			var settings = require('settings'),
			  ...
			  return componentsDirecotry + '/co-textarea/' + relativePath;
		})
},{"./src/js/textarea":10,"component":4,"rivets":"rivets","settings":"settings"}],2:[function(require,module,exports){
"use strict";
...
var IS_IPAD = !!~userAgent.toLowerCase().indexOf("ipad"); //for fixes for ipad
var IS_IE11 = !!userAgent.match(/Trident\/7\./); //for fixes for ie11
var IS_EDGE = /Edge\/\d./i.test(userAgent); //for fixes for Edge
...
var Textarea = exports.Textarea = function (_Component) {
	...
	_createClass(Textarea, [{
		...
		value: function init() {
			...
			Object.keys(this.defaults).forEach(this.refresh, this);

			(0, _nav.onenter)(function () {
				// fix bug in .ipa offset the cursor by swipe
		...
		value: function addEvents() {
			...
			['error', 'invalid'].forEach(function (event) {
				_this2.textarea.addEventListener(event, function (event) {
					if (!event.bubbles) {
						(0, _utils.dispatchEvent)(_this2.element, event.type, (0, _utils.mixin)(_this2.dispatchOptions, event));
					}
				});
			});

			this.textarea.addEventListener('input', function () {
				_this2.value = _this2.textarea.value;

				if (!_this2.isFocus) {
					_this2.isFocus = true;
					(0, _utils.dispatchEvent)(_this2.element, 'focus', _this2.dispatchOptions);
				}
			});
			this.textarea.addEventListener('keydown', function () {
				return _this2.correctCursor();
			});
			...
			events.forEach(function (eventName) {
				document.addEventListener(eventName, function (event) {
					if (event.target === _this2.textarea || event.target === _this2.element) {
						_this2.focusElement();
					} else {
						_this2.blurElement();
					}
		...
		value: function focusElement() {
			...
			if (!this.isFocus) {
				...
				setTimeout(function () {
					// fix bug in .ipa suspension by double tap
					_this4.input.focus();
					_this4.textarea.focus();

					(0, _utils.dispatchEvent)(_this4.element, 'focus', _this4.dispatchOptions);
				}, 0);
			}
		}
	}, {
		key: 'blurElement',
		value: function blurElement(isBeforeScroll) {
			if (this.isFocus || isBeforeScroll) {
				this.isFocus = false;
				window.focus();
				this.textarea.blur();
				(0, _utils.dispatchEvent)(this.element, 'blur', this.dispatchOptions);
			}
		}
	}, {
		key: 'normalizeValue',
		value: function normalizeValue(value, maxLength) {
		...
		key: 'refresh',
		value: function refresh(attribute) {
...
},{"../template.html":11,"component":4,"nav":"nav","touch":"touch","utils":"utils"}],11:[function(require,module,exports){
module.exports = '<textarea co-textarea=""></textarea>'
},{}]},{},[1]);
